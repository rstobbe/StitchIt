...
TrajNumsOverAves = ReconObj.TrajNumsOverAves;
...
if DispFigs > 0
    figure(2001); hold on; 
    plot(AcqsArr(logical(ExpInds)),NavSig(logical(ExpInds)),'rx')
    plot(AcqsArr(logical(PeriExpInds)),NavSig(logical(PeriExpInds)),'gx')
end
...
PeriVals = 0;
for n = 1:NumTraj
    Weight(n,:) = ExpInds(TrajNumsOverAves(n,:));
    SumWeight(n) = sum(Weight(n,:),2);
    if SumWeight(n) == 0
        PeriVals = PeriVals + 1;
        Weight(n,:) = PeriExpInds(TrajNumsOverAves(n,:));
        SumWeight(n) = sum(Weight(n,:),2);
    end
    if SumWeight(n) == 0
        Holes = Holes + 1;
