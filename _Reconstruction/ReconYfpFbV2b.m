%==================================================================
% (V2b)
%   - Navigator flip is an input
%==================================================================

classdef ReconYfpFbV2b < handle

properties (SetAccess = private)                   
    Method = 'ReconYfpFbV2b'
    BaseMatrix
    AcqInfo
    AcqInfoRxp
    Rcvrs
...
% Constructor
%==================================================================  
function ReconObj = ReconYfpFbV2b()              
...
function [Image,err] = CreateImage(ReconObj,DataObj)     
    ...
    Image = [];
    ReconObj.DispStatObj.Status('ReconYfpFbV2b',1);
    ...
    %% Info
    if isfield(DataObj.DataInfo.ExpPars.Sequence,'NumImages')
        NumImages = DataObj.DataInfo.ExpPars.Sequence.NumImages;
    else
        NumImages = 1;
    end
    if isfield(DataObj.DataInfo.ExpPars.Sequence,'Dummies')
    ...
    %% First Data-points (for Navigator)
    ...
    for n = 1:NumImages
        ...
        TrajMashObjArray(n).SetFlip(ReconObj.NavigatorFlip(n));
        ...
        if isfield(DataObj.DataInfo.ExpPars.Sequence,'trnav')
...
function SetNavigatorFlip(ReconObj,val)    
    ReconObj.NavigatorFlip = val;
end
function SetDataTest(ReconObj,val)    
    ReconObj.DataTest = val;
end
