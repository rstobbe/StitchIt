...
% Constructor
%==================================================================  
function ReconObj = ReconYafiV1a()              
...
function [Image,err] = CreateImage(ReconObj,DataObjArr)     
    ...
    ReconObj.DispStatObj.Status('ReconYafiV1a',1);
    ...
    Image = zeros([ReconObj.BaseMatrix,ReconObj.BaseMatrix,ReconObj.BaseMatrix,DataObj0.RxChannels,length(DataObjArr),3],'like',DataType);
    for n = 1:length(DataObjArr)
        ...
        if ReconObj.UseExternalShift
            ...
            Data = DataObjArr{n}.DataObj.ReturnAllYafiDataWithShift(ReconObj.AcqInfo{ReconObj.ReconNumber},ReconObj.ReconNumber);
        end
        ...
        for m = 1:2
            if ReconObj.OffResCorrection
                error('not coded');
                %Image(:,:,:,:,n,m) = StitchIt.CreateImage(Data,RxProfs,OffResMapInt,OffResTimeArr);
            else
        ...
        Image(:,:,:,:,n,3) = acos((TrRat*ImRat-1)./(TrRat-ImRat));
