...
% Constructor
%==================================================================  
function ReconObj = ReconYvfaV2a()              
...
function [Image,err] = CreateImage(ReconObj,DataObj)     
    ...
    %--
    % figure(1235); hold on;
    % plot(squeeze(mean(abs(DataFull(1000,1:500,1)),3)));
    % plot(squeeze(mean(abs(DataFull(1,1:500,1)),3)));
    %--

    sz = size(DataFull);
    YvfaData = zeros(NumTraj,sz(2),sz(3),NumFlips,'single');
    for n = 1:NumFlips
        YvfaData(:,:,:,n) = DataFull(TrajPerFlip*(n-1)+Dummies+1:TrajPerFlip*n,:,:);
    end
    ...
    YvfaDataRxProf = DataObj.ScaleData(StitchIt,YvfaDataRxProf);
    ReconObj.DispStatObj.Status('Generate',3);
    RxProfs = StitchIt.CreateImage(YvfaDataRxProf);
    %--
    %ReconObj.DispStatObj.SetDisplayRxProfs(1);
    %--
    ReconObj.DispStatObj.TestDisplayRxProfs(RxProfs);
    clear('StitchIt','YvfaDataRxProf');
    ...
    if ReconObj.DoSaveSmallerFov
        ...
        % Image = zeros([Sz(1),Sz(2),Sz(3),DataObj.RxChannels,1,NumFlips],'like',single(1+1i)); 
        Image = zeros([Sz(1),Sz(2),Sz(3),1,1,NumFlips],'like',single(1+1i)); 
    else
        % Image = zeros([ReconObj.BaseMatrix,ReconObj.BaseMatrix,ReconObj.BaseMatrix,DataObj.RxChannels,1,NumFlips],'like',single(1+1i));
        Image = zeros([ReconObj.BaseMatrix,ReconObj.BaseMatrix,ReconObj.BaseMatrix,1,1,NumFlips],'like',single(1+1i));
    end
    YvfaData = DataObj.ScaleData(KernHolder,YvfaData);  
    for n = 1:NumFlips    
        ReconObj.DispStatObj.Status(['Generate ',num2str(n)],3);
        ImageOut = StitchIt.CreateImage(YvfaData(:,:,:,n));
        if ReconObj.DoSaveSmallerFov
