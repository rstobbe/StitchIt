...
% Constructor
%==================================================================  
function ReconObj = ReconYdabV1a()              
...
function [Image,err] = CreateImage(ReconObj,DataObjArr)     
    ...
    ReconObj.DispStatObj.Status('ReconYdabV1a',1);
    ...
    % plot(abs(FirstDataPoints(1:1:end,1)));
    % plot(real(FirstDataPoints(1:1:end,1)));
    % plot(imag(FirstDataPoints(1:1:end,1)));
    % Fh = figure(12345); hold on;
    % plot(angle(FirstDataPoints(1:1:end,1)));
    AllData = DataObj0.ReturnYvf2Data(ReconObj.AcqInfo{1},1);
    MeanData = squeeze(mean(AllData,1));
    figure(2346); hold on;
    plot(abs(MeanData(:,:,1)));
    figure(2347); hold on;
    plot(abs(MeanData(:,:,2)));
    ...
    if ReconObj.ObjectAtIso
        Data = DataObj0.ReturnYvf2Data(ReconObj.AcqInfoRxp,[]);
    else
    ...
    ReconObj.DispStatObj.Status('Load Data',3);
    if ReconObj.ObjectAtIso
        Data = DataObj0.ReturnYvf2Data(ReconObj.AcqInfo{1},1);
    else
        % if ReconObj.UseExternalShift
        %     Data = DataObj0.ReturnDataSetWithExternalShift(ReconObj.AcqInfo{1},1,ReconObj.Shift);
        % else
        %     Data = DataObj0.ReturnDataSetWithShift(ReconObj.ReconObj.AcqInfo{1},[]);
        % end
    end
    ...
    ImageMasked = Image;
    ImageMasked(abs(ImageMasked) < 0.01*max(abs(ImageMasked(:)))) = NaN;
    ImRat = abs(ImageMasked(:,:,:,:,:,1))./abs(ImageMasked(:,:,:,:,:,2));
    Val = 1./(2*ImRat);
    Val(Val > 1) = NaN;
    Val(Val < -1) = NaN;
    ImFlip = acos(Val);
    RelB1Map = (ImFlip*180/pi)/60;
