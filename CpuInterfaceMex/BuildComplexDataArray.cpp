...
float *Data;
Data = (float*)mxGetComplexSingles(plhs[0]);
...
for (int i=0;i<DataMemPosArrLen;i++){
    ...
    for (int j=0;j<DataCha;j++){
        for (int k=0;k<DataCol;k++){
            Data[(j*2*DataCol*DataBlockSize)+(i*2*DataCol)+(2*k)] = ReadArray[(j*DataReadSize)+DataStart+(2*k)];
            Data[(j*2*DataCol*DataBlockSize)+(i*2*DataCol)+(2*k)+1] = ReadArray[(j*DataReadSize)+DataStart+(2*k)+1];
        }
    }    
}
