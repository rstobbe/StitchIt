...
function [ExpPars,PanelOutput,err] = YthmpB1_v2k_SeqDat(MrProt,DataInfo)
...
ExpPars.Sequence.RfSatFlip = test1{22};
ExpPars.Sequence.RfSatDur = test1{23};
ExpPars.Sequence.RfSatTr = test1{24};
ExpPars.Sequence.RfSatIntv = test1{25};
ExpPars.Sequence.RfSatGsDur = test1{26};
ExpPars.Sequence.RfSatGsMag = test1{27};
ExpPars.Sequence.RfSatGsSlewRate = test1{28};
ExpPars.Sequence.RfSatGsRnd = test1{29};
if isempty(ExpPars.Sequence.RfSatGsRnd)
    ExpPars.Sequence.RfSatGsRnd = 0;
end
ExpPars.Sequence.Dummies = test1{30};
...
for n = 1:ExpPars.Sequence.NumImages
    m = m+1;
    Panel(m,:) = {'Flip (degrees)',ExpPars.Sequence.flip(n),'Output'};
end
m = m+1;
Panel(m,:) = {'','','Output'};
for n = 1:ExpPars.Sequence.NumImages
...
m = m+1;
Panel(m,:) = {'RfSatFlip (degrees)',ExpPars.Sequence.RfSatFlip,'Output'};
m = m+1;
Panel(m,:) = {'RfSatDur (us)',ExpPars.Sequence.RfSatDur,'Output'};
m = m+1;
Panel(m,:) = {'RfSatTr (us)',ExpPars.Sequence.RfSatTr,'Output'};
m = m+1;
Panel(m,:) = {'RfSatIntv (us)',ExpPars.Sequence.RfSatIntv,'Output'};
m = m+1;
Panel(m,:) = {'RfSatGsDur (us)',ExpPars.Sequence.RfSatGsDur,'Output'};
m = m+1;
Panel(m,:) = {'RfSatGsMag (mT/m)',ExpPars.Sequence.RfSatGsMag,'Output'};
m = m+1;
Panel(m,:) = {'RfSatGsSlewRate (mT/m/ms)',ExpPars.Sequence.RfSatGsSlewRate,'Output'};
m = m+1;
Panel(m,:) = {'RfSatGsRnd',ExpPars.Sequence.RfSatGsRnd,'Output'};
m = m+1;
