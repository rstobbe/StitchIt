...
function [ExpPars,PanelOutput,err] = Yvf2Hrd_v2k_SeqDat(MrProt,DataInfo)
...
ExpPars.Sequence.rdwn = test1{14};
if isempty(ExpPars.Sequence.rdwn)
    ExpPars.Sequence.rdwn = 0;
end
ExpPars.Sequence.trbuf = test1{15};
if isempty(ExpPars.Sequence.trbuf)
    ExpPars.Sequence.trbuf = 0;
end
ExpPars.Sequence.GradDur = test1{17};
ExpPars.Sequence.GradMag = test1{18};
if isempty(test1{19})
    test1{19} = 0;
end
if test1{19} == 0
    ...
    ExpPars.Sequence.RandRelWid = test1{20}/100;
    ExpPars.Sequence.GradRandSeed = test1{23};
end
if isempty(test1{21})
    test1{21} = 0;
end
...
ExpPars.Sequence.GradSlewRate = test1{22};
if isempty(test1{24})
    test1{24} = 0;
end
if test1{24} == 0
...
Panel(9,:) = {'','','Output'};
Panel(10,:) = {'RfDur1 (us)',ExpPars.Sequence.rfpulselen(1),'Output'};
Panel(11,:) = {'RfDur2 (us)',ExpPars.Sequence.rfpulselen(2),'Output'};
Panel(12,:) = {'RingDown (us)',ExpPars.Sequence.rdwn,'Output'};
Panel(13,:) = {'TrBuf (us)',ExpPars.Sequence.trbuf,'Output'};
Panel(14,:) = {'','','Output'};
Panel(15,:) = {'GradDur (us)',ExpPars.Sequence.GradDur,'Output'};
Panel(16,:) = {'GradMag (mT/m)',ExpPars.Sequence.GradMag,'Output'};
Panel(17,:) = {'RandGradSpoil',ExpPars.Sequence.RandGradSpoil,'Output'};
Panel(18,:) = {'RandGradRelWid',ExpPars.Sequence.RandRelWid,'Output'};
Panel(19,:) = {'RandGradSeed',ExpPars.Sequence.GradRandSeed,'Output'};
Panel(20,:) = {'GradSlewRate (mT/m/ms)',ExpPars.Sequence.GradSlewRate,'Output'};
Panel(21,:) = {'MultiAxisSGradSpoil',ExpPars.Sequence.MultiAxisGradSpoil,'Output'};
Panel(22,:) = {'RfSpoil',ExpPars.Sequence.RfSpoil,'Output'};
Panel(23,:) = {'Gain',ExpPars.Sequence.Gain,'Output'};
% Panel(24,:) = {'','','Output'};
